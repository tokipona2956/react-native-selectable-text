...
NSString *const RNSTV_CUSTOM_SELECTOR = @"_RNSTV_CUSTOM_SELECTOR_";
...
-(void) _handleGesture
{
    ...
    for(NSString *menuItemName in self.menuItems) {
        NSString *sel = [NSString stringWithFormat:@"%@%@", RNSTV_CUSTOM_SELECTOR, menuItemName];
...
- (void)forwardInvocation:(NSInvocation *)invocation
{
    ...
    NSRange match = [sel rangeOfString:RNSTV_CUSTOM_SELECTOR];
    if (match.location == 0) {
